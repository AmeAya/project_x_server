...
class FilterApiView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        from project.down_ex import dowlond_exe
        wb = dowlond_exe(request)
        return Response(data=RecordsSerializer(filters_test(request), many=True).data, status=status.HTTP_200_OK)
